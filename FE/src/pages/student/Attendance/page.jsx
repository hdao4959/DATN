...
const ShowStudentAttendance = () => {
    ...
    const { data, isLoading } = useQuery({
        queryKey: ["LIST_CLASSES", semesterCode],
        queryFn: async () => {
            const response = await api.get("/student/attendances", {
                params: { search: semesterCode || semesterCodeDefault },
            });
    ...
    useEffect(() => {
        if (data?.attendances) {
            data.attendances.forEach((subjectData, index) => {
        ...
        <div>
            <div className="card" style={{ minHeight: "800px" }}>
                <div className="card-header">
                    ...
                    >
                        <select
                            value={semesterCode || semesterCodeDefault}
                ...
                <div className="card-body">
                    {isLoading ? (
                        ...
                        data?.attendances?.map((classData, index) => (
                            <div
                                key={index}
                                className="attendance-table table-striped card-body border-spacing-3"
                                style={{
                            ...
                            >
                                ...
                                >
                                    <h3
                                        ...
                                        style={{ color: "black" }}
                                    >
                                        #{index + 1} Lớp: {classData.class_name}{" "}
                                        - Môn: {classData.subject_name}
                                    </h3>
                                    <div>
                                        Vắng{" "}
