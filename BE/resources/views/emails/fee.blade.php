...
<body>
    ...
    <p><strong>3. Thời hạn nộp tiền:</strong> Từ ngày <strong>{{ {{ \Carbon\Carbon::parse($data['start_date'])->format('d/m/Y') }}
    }}</strong> đến trước 24h ngày <strong>{{ {{ \Carbon\Carbon::parse($data['due_date'])->format('d/m/Y') }}
}}</strong></p>

    <p><strong>4. Đối với sinh viên thiếu nợ môn Tiếng Anh:</strong> Chủ động đăng ký trả nợ môn, sử dụng số dư Ví Học Lại (nếu có).</p>

    <p><strong>5. Hình thức nộp tiền:</strong> Sinh viên truy cập hệ thống <a href="https://dng.fpt.edu.vn">dng.fpt.edu.vn</a> để xem hướng dẫn nộp tiền.</p>

    <h2>Hướng dẫn tra cứu học phí đã nộp</h2>
    <ul>
        <li>Hóa đơn sẽ được gửi về địa chỉ Email sinh viên đã đăng ký. Thông thường hóa đơn sẽ được gửi trong vòng 7 ngày kể từ thời điểm thanh toán thành công.</li>
        <li>Khi sinh viên hoàn thành học phí, phòng Hành Chính sẽ xuất hóa đơn và đẩy tiền lên ví Học Phí. Ví Học Phí hiển thị tiền => nộp học phí thành công.</li>
    </ul>

    <h2>Lưu ý</h2>
    <ul>
        <li>Các trường hợp không nộp học phí đúng thời gian quy định được coi như tự nguyện thôi học.</li>
        <li>Sinh viên đã thôi học có nguyện vọng tiếp tục học phải làm thủ tục nhập học trở lại, hồ sơ bao gồm Đơn xin nhập học trở lại và phí nhập học trở lại 500,000 VNĐ.</li>
    </ul>
